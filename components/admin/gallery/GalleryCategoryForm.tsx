...
export function GalleryCategoryForm({ mode, initial }: Props) {
  const router = useRouter()
  const t = useTranslations('gallery.categories.form')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const url = mode === 'create' ? '/api/gallery/categories' : `/api/gallery/categories/${initial!.id}`
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 bg-white border border-gray-200 rounded-xl p-4 sm:p-6">
      <LangTabs
        ...
        label={t('name')}
        placeholder={{ de: 'Restaurant', en: 'Restaurant', uk: 'Ресторан' }}
        requiredDE
      />
